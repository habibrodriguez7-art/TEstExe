...
local SCRIPT_URL = "https://raw.githubusercontent.com/4LynxX/Lynx/refs/heads/main/LynxxMain.lua"
...
do
    ...
    do
        ...
        -- =========================================================
        -- TELEPORT NOW: teleport ke atas permukaan air (Y tinggi)
        -- di atas lokasi Sunken Wreckage, tanpa masuk event loop
        -- =========================================================
        local SURFACE_Y_OFFSET = 80 -- tinggi di atas posisi wreckage (di atas air)

        local function teleportAboveTreasure()
            local sw = workspace:FindFirstChild("Sunken Wreckage")
            if not sw then
                setStatus("Sunken Wreckage tidak ditemukan!")
                task.spawn(function()
                    task.wait(2)
                    setStatus(autoTreasureRunning and "Waiting for Treasure Hunt..." or "Inactive")
                end)
                return
            end

            local swPos = sw:GetPivot().Position
            local targetPos = Vector3.new(swPos.X, swPos.Y + SURFACE_Y_OFFSET, swPos.Z)
            local targetCF = CFrame.new(targetPos)

            local char = LocalPlayer.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            if not hrp then
                setStatus("Character tidak ditemukan!")
                return
            end

            setStatus("Teleporting di atas Treasure Hunt...")
            ...
            setStatus("Teleport selesai! (di atas lokasi event)")
            task.spawn(function()
                task.wait(2.5)
                setStatus(autoTreasureRunning and "Waiting for Treasure Hunt..." or "Inactive")
            end)
        ...
        -- =========================================================
        -- PROCESS EVENT: ambil semua chest dengan prompt lebih cepat
        -- =========================================================
        local function processTreasureEvent()
            ...
            while autoTreasureRunning do
                ...
                setStatus("Claiming Treasure (" .. #chests .. " remaining)...")
                ...
                -- Cari prompt lebih cepat: max 8 attempt, jeda 0.1s
                local prompt = nil
                for attempt = 1, 8 do
                    prompt = findPrompt(targetChest)
                    if prompt then break end
                    -- Pastikan posisi tetap benar selama nunggu prompt
                    pcall(function()
                        hrp.CFrame = chestCF
                        hrp.AssemblyLinearVelocity  = Vector3.zero
                        hrp.AssemblyAngularVelocity = Vector3.zero
                    end)
                    task.wait(0.1) -- lebih cepat dari 0.3s sebelumnya
                end
                ...
                if prompt then
                    ...
                    -- Pastikan posisi pas sebelum fire
                    pcall(function()
                        hrp.CFrame = chestCF
                        hrp.AssemblyLinearVelocity  = Vector3.zero
                        hrp.AssemblyAngularVelocity = Vector3.zero
                    ...
                    -- Fire prompt & retry agresif selama chest belum Opened
                    local waitStart = tick()
                    local fireInterval = 0.15 -- fire ulang tiap 0.15s (jauh lebih cepat)
                    local lastFire = 0
                    while not targetChest:GetAttribute("Opened") and (tick() - waitStart) < 6 do
                        local now = tick()
                        -- Re-fire prompt setiap fireInterval detik
                        if now - lastFire >= fireInterval then
                            pcall(fireproximityprompt, prompt)
                            lastFire = now
                        end
                        -- Jaga posisi selama menunggu
                        pcall(function()
                            if hrp and hrp.Parent then
                                hrp.CFrame = chestCF
                                hrp.AssemblyLinearVelocity  = Vector3.zero
                        ...
                        task.wait(0.05) -- polling lebih ketat
                    end
                    task.wait(0.15) -- cooldown singkat sebelum ke chest berikutnya
                else
                    -- Prompt tidak ketemu, skip chest ini dan lanjut
                    pcall(function() hrp.Anchored = false end)
                    task.wait(0.5) -- dikurangi dari 1s
                end
        ...
        TreasureSection:AddButton({
            Title    = "Teleport Now (di atas lokasi)",
            Callback = function()
                task.spawn(teleportAboveTreasure)
            end,
        })
