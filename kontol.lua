...
do
    ...
    local HeartbeatConn2    = nil
    local VisibilityConn2   = nil

    local function initAutoSkillcheck()
        task.spawn(function()
            local Players       = game:GetService("Players")
            local LocalPlayer   = Players.LocalPlayer
            local RunService    = game:GetService("RunService")
            ...
            local platform = LocalPlayer:GetAttribute("platform") or "PC"
            local isMobile = platform == "Mobile" or UserInputService.TouchEnabled
            local TouchID  = 8822
            ...
            local function triggerInput()
                if isMobile then
                    ...
                    if cur and cur:IsA("GuiObject") then
                        local p2, s2, i2 = cur.AbsolutePosition, cur.AbsoluteSize, GuiService:GetGuiInset()
                        local cx = p2.X + (s2.X / 2) + i2.X
                        local cy = p2.Y + (s2.Y / 2) + i2.Y
                        pcall(function()
            ...
            if VisibilityConn2 then VisibilityConn2:Disconnect() end
            VisibilityConn2 = check:GetPropertyChangedSignal("Visible"):Connect(function()
                if not skillcheckEnabled then return end
                local team = LocalPlayer.Team and LocalPlayer.Team.Name or ""
                if team == "Survivors" and check.Visible then
                    if HeartbeatConn2 then HeartbeatConn2:Disconnect() end
                    HeartbeatConn2 = RunService.Heartbeat:Connect(function()
                        if not skillcheckEnabled then
                            HeartbeatConn2:Disconnect()
                            HeartbeatConn2 = nil
                            return
                        end
                        local lr = line.Rotation % 360
                        local gr = goal.Rotation % 360
                        -- Perfect zone: tengah goal
                        local ss = (gr + 101) % 360
                        local se = (gr + 115) % 360
                        local inZone = (ss > se and (lr >= ss or lr <= se)) or (lr >= ss and lr <= se)
                        if inZone then
                            triggerInput()
                            if HeartbeatConn2 then
                                HeartbeatConn2:Disconnect()
                                HeartbeatConn2 = nil
                            end
                        end
                    end)
                elseif HeartbeatConn2 then
                    HeartbeatConn2:Disconnect()
                    HeartbeatConn2 = nil
                end
            end)
    ...
    local s = SurTab:AddSection("Auto Skillcheck Perfect")
    ...
    s:AddToggle({
        ...
        Callback = function(v)
            ...
            if v then
                ...
                if HeartbeatConn2 then HeartbeatConn2:Disconnect() HeartbeatConn2 = nil end
                if VisibilityConn2 then VisibilityConn2:Disconnect() VisibilityConn2 = nil end
            end
        end,
    })
end
